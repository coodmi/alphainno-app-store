...
export default async function GameDetailPage({ params }: GameDetailProps) {
  ...
  const relatedProducts = games
    ...
    .filter((item) => item.category === game.category || item.platform === game.platform)
    .slice(0, 6);
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 lg:py-10 space-y-8">
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500 inline-flex items-center gap-1">
                  ...
                  {game.rating.toFixed(1)} • {game.platform}
                </p>
              </div>
              <button className="shrink-0 bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-blue-700 transition">
                {game.price === "Free" ? "Install" : `Buy ${game.price}`}
              </button>
            </div>
          </section>

          <Link href="/games" className="text-sm text-blue-600 hover:underline inline-flex items-center gap-1">
            <span>←</span>
          ...
          <section className="sticky top-[68px] z-20 rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
            <nav className="flex items-center gap-5 overflow-x-auto text-sm">
              <a href="#overview" className="text-gray-700 font-semibold hover:text-blue-600 transition whitespace-nowrap">Overview</a>
              <a href="#screenshots" className="text-gray-600 hover:text-blue-600 transition whitespace-nowrap">Screenshots</a>
              <a href="#reviews" className="text-gray-600 hover:text-blue-600 transition whitespace-nowrap">Reviews</a>
              <a href="#requirements" className="text-gray-600 hover:text-blue-600 transition whitespace-nowrap">Requirements</a>
            </nav>
          </section>

          <section id="overview" className="grid lg:grid-cols-[1fr_300px] gap-6 scroll-mt-40">
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-start gap-5">
                ...
                <div className="space-y-3">
                  <div>
                    <h1 className="text-3xl lg:text-4xl font-bold tracking-tight">{game.name}</h1>
              ...
              <p className="text-gray-700 leading-relaxed max-w-3xl">{game.description}</p>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">Rating</p>
                  <p className="text-base font-semibold">{game.rating.toFixed(1)} / 5</p>
                </div>
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                ...
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                  ...
                  <p className="text-base font-semibold">{game.price}</p>
                </div>
              </div>
            </div>
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  {[1, 2, 3].map((item) => (
                    <div key={item} className="relative aspect-[10/16] rounded-xl overflow-hidden border border-gray-200 bg-gray-100">
                      <Image src={game.imageUrl} alt={`${game.name} screenshot ${item}`} fill className="object-cover" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div id="reviews" className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm scroll-mt-40">
                ...
                <div className="grid md:grid-cols-[180px_1fr] gap-6">
                  <div className="rounded-xl bg-gray-50 border border-gray-200 p-4 text-center">
                    <p className="text-4xl font-bold">{game.rating.toFixed(1)}</p>
                    <div className="flex justify-center gap-1 my-2">
                      {[...Array(5)].map((_, index) => (
                        <Star
                          key={index}
                          size={16}
                          className={index < Math.floor(game.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}
                        />
                      ))}
                    </div>
            ...
            <aside className="space-y-6">
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/games" className="text-xs text-blue-600 hover:underline">
                    Discover more
                  </Link>
                </div>
                ...
                {relatedProducts.length > 0 ? (
                  ...
                  <div className="text-sm text-gray-500 rounded-lg border border-gray-200 p-3">No related games found.</div>
                )}
              </div>
              ...
              <div id="requirements" className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm scroll-mt-40">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between gap-2">
                    ...
                    <span className="font-medium text-right">{game.platform} 11 or later</span>
                  </div>
                  <div className="flex justify-between gap-2">
